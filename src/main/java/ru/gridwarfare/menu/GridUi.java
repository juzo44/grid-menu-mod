 ...
 * Все цвета, шрифты, утилиты рендеринга.
 * Иконки — через UiIcons (векторные), НЕ текстуры.
 */
public final class GridUi {
    ...
    public static void background(GuiGraphics g, int w, int h) {
        ...
        for (int i = 0; i < edgeBands; i++) {
            ...
            int c = a << 24;
            g.fill(0, i, w, i + 1, c);
            g.fill(0, h - 1 - i, w, h - i, c);
            g.fill(i, 0, i + 1, h, c);
            g.fill(w - 1 - i, 0, w - i, h, c);
        }
        ...
        for (int i = 0; i < cornerBands; i++) {
            ...
            if (sz < 1) continue;
            g.fill(0, 0, sz, sz, c);
            g.fill(w - sz, 0, w, sz, c);
            g.fill(0, h - sz, sz, h, c);
            g.fill(w - sz, h - sz, w, h, c);
        }
    }

    /* ═══════════════════════════
       БРЕНД-МАРКА (32×32, clip-path polygon, font 17px/900)
       ═════════════════════════ */
